...
GENOME_DIR = '/media/data/sxv/genome/gencode39-68/'
GTF_FILE = '/media/data/sxv/genome/gencode.v39.annotation.gtf'
...
def make_fastq(batch_dir):
    ...
    if not glob(os.path.join(fastq_dir, '*.gz')):
        ...
        run(f'mv *.gz fastq', cwd=batch_dir)
...
def align(batch_dir):
    ...
    if glob(os.path.join(fastq_dir, '*.gz')) and not glob(os.path.join(bam_dir, '*.bam')):
        ...
        run(f'bash {os.path.basename(ALIGN)} -d {GENOME_DIR} *.gz', cwd=fastq_dir)
