...
def align():
  if glob('%s/fastq/*gz' % batch_dir) and not glob('%s/bam/*bam' % batch_dir):
    ...
    os.system("bash -c 'cd %s/fastq && bash %s -d /media/data/sxv/genome/gencode39-68/ *fastq.gz'" % (batch_dir, ALIGN))
...
def make_expression_table():
  if glob('%s/bam/*bam' % batch_dir) and not glob('%s/bam/gene_expression.xlsx' % batch_dir):
    ...
    os.system("bash -c 'cd %s/bam && Rscript %s --no-rlog /media/data/sxv/genome/gencode.v39.annotation.gtf *bam'" % (batch_dir, MAKE_EXPRESSION_TABLE))
